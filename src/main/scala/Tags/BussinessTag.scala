...
import Utils.{AmapUtil, JedisConnectionPool, String2Type, TagInterface}
...
/**
  * 商圈标签
  */
object BussinessTag extends  TagInterface{
  override def makeTags(args: Any*): List[(String, Int)] = {
    ...
    // 获取经纬度，过滤经纬度
    if(String2Type.toDouble(long)>= 73.0 &&
      String2Type.toDouble(long)<= 135.0 &&
      String2Type.toDouble(lat)>=3.0 &&
      String2Type.toDouble(lat)<= 54.0){
      // 先去数据库获取商圈
